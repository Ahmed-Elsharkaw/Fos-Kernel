...
struct Env* fos_scheduler_MLFQ()
{
	...
	if(curenv !=NULL )
	{
		if(glopal!=num_of_ready_queues-1)
		{
			//1.	Set the "next_env" by the found environment
		enqueue(&env_ready_queues[glopal+1],curenv);
		}
		else //if(glopal==num_of_ready_queues-1)
		{
		enqueue(&env_ready_queues[glopal],curenv);
		}
