...
{
	//المطالب
		//create_page_table(): should create new page table and link it to the directory. REMEMBER TO:
		//clear all entries (as it may contain garbage data)
		//clear the TLB cache (using "tlbflush()")
	uint32 MYPage_size=PAGE_SIZE;//4KB
	uint32* physicaladdress;
	uint32* pagetable_pysical_address;
	uint32 Zero_Variable=0;
	//هكريت نيو بيج تيبل بستخدام الهيب لان البيج تيبل عرباره عن حاجه زي الاراي كده
	uint32* return_pointer=kmalloc(MYPage_size);
		    //هنا بعمل كلير لكل values
		    //وبحط مكنها اصفار بمسح كل الانتريس
	memset((uint32*)return_pointer , Zero_Variable, MYPage_size);

		    //هبدا اعمل اللينك بين البيج تيبل والديريكتوري تايبل
	int dirctorytableindex=PDX(virtual_address);
		    //phy for directory in the ram
	physicaladdress=&ptr_page_directory[dirctorytableindex];
	pagetable_pysical_address=(uint32*)kheap_physical_address((uint32)return_pointer);

	*physicaladdress = ((uint32)pagetable_pysical_address |PERM_USER|PERM_PRESENT| PERM_WRITEABLE);
			//طبعا موجود في الديسكريبشن
	 tlbflush();
...
int j=0;
	int i=0;
	int limit=size/ PAGE_SIZE;// 3 4 5 //12KB +++==> 3
	//before start :
	//In allocateMem(), all pages you allocate """"will not be added to the working set"""
	//(not exist in main memory).
	//Instead, they should be added to the page file,
	//so that when this page is accessed, a page fault will load it to memory.
	//Add a new environment page with the given virtual address
			//to the page file and initialize it by zeros.
	while(i<limit)
		{
			//Add a new environment page with the given virtual address
			//to the page file and initialize it by zeros.
			int zero_or_Error = pf_add_empty_env_page(e, virtual_address, 0);
				if(zero_or_Error==E_NO_PAGE_FILE_SPACE)
				{
					panic("no enough ");
				}
...
// av ++> start P
  //This function should:
	...
	struct Frame_Info* my_fram_info = NULL;
	uint32* POINTER_PT;
	int LIMIT=size/PAGE_SIZE;//12 KB =3 pages
	uint32 LOGICAL_ADDRESS=virtual_address;

	//1. Free ALL pages of the given range from the Page File:
	int k=0;
	while(k<LIMIT)
	{//Remove an existing environment page at the given virtual address from the page file.
		pf_remove_env_page(e,LOGICAL_ADDRESS);
		LOGICAL_ADDRESS=LOGICAL_ADDRESS+PAGE_SIZE;
		k++;
	}
	...
	int j=0;
	LOGICAL_ADDRESS=virtual_address;
	int MAX_SIZE_FOR_WS=e->page_WS_max_size;
	int SSS_END =ROUNDUP((virtual_address+size),PAGE_SIZE);
	while(j<MAX_SIZE_FOR_WS)
	{
		// j is entry index
		uint32	current_address_in_working_set=env_page_ws_get_virtual_address(e,j);

	    if(current_address_in_working_set>=LOGICAL_ADDRESS && current_address_in_working_set<SSS_END)
	    {
	    	uint32 x=ROUNDUP(virtual_address +size,PAGE_SIZE);
	    	//uint32 total=LOGICAL_ADDRESS+x;
	    	if(current_address_in_working_set<x && current_address_in_working_set>=virtual_address)
	    	  {
	    		unmap_frame(e->env_page_directory,(void*)current_address_in_working_set);
	...
	//*****//*****//*****//*****//*****////*****////*****////
	//3. Removes ONLY the empty page tables (i.e. not used)
	//(no pages are mapped in the table):
	LOGICAL_ADDRESS=virtual_address;
	int i=0;
	while(i<LIMIT)
	{
		POINTER_PT=NULL;
		int F=1;
		get_page_table(e->env_page_directory,(void*)LOGICAL_ADDRESS,&POINTER_PT);
		if(POINTER_PT !=NULL)
		{
			int counter=0;
			int number_of_pages_in_page_table=1024;
			while(counter<number_of_pages_in_page_table)
			{
				if(POINTER_PT[counter]!=0)
				{
					F=0;
					break;
				}
			...
			if(F==1)
			{
				kfree((void*)POINTER_PT);
				pd_clear_page_dir_entry(e,(uint32)LOGICAL_ADDRESS);
			}
		LOGICAL_ADDRESS=LOGICAL_ADDRESS+PAGE_SIZE;
		}
...
{
	//[PROJECT 2022 -- DynamicDeAlloc] freeMem() [Kernel Side]
